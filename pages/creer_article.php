...
$show = new \app\views\Article();
$art = new \app\controllers\Article();
...
<main>
  <?php if (isset($_SESSION['user']) && !empty($_SESSION['user']->getId()) && $_SESSION['user']->getDroits() === 42) : ?>
    <section id="pageInscription">
      ...
      <div id="formCreer">
        ...
        <br>
        <form id="blogForm" action="creer_article.php<?php if (isset($modifart)) :
                                                        echo "?modifart={$_GET['modifart']}";
                                                      endif; ?>" method="POST">
          <br>
          <div>
            ...
            <input type="text" id="titre" name="titre" value="<?php
                                                              if (isset($modifart)) : echo "{$modifart['titre']}";
                                                              endif;
                                                              ?>">
          </div>
          <br>
          <div>
            <label for="article" class="form-label">Article
              :</label><br>
            <textarea name="article" rows="5" cols="30" placeholder="Votre texte ici ..."><?php
                                                                                          if (isset($modifart)) : echo "{$modifart['article']}";
                                                                                          endif; ?></textarea>
          </div>
          <br>
          <div>
          ...
          if (isset($_POST['envoyer'])) {
            $post = new \app\controllers\Article();
